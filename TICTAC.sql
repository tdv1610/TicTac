...
INSERT INTO THUCHIEN VALUES ('abcd1234@gmail.com.vn', '6','2', 'C?n làm');
--------------------------------------------------------------------------------
...
BEGIN
    ...
    IF L_EMAIL IS NULL THEN
        DBMS_OUTPUT.PUT_LINE('User khong ton tai');
    ELSE
        -- Xóa t?t c? các b?n ghi liên quan ??n ng??i dùng này tr??c
        XOA_THUCHIEN_THEO_EMAIL(I_EMAIL);
        XOA_NHOM_THEO_EMAIL(I_EMAIL);
        XOA_NHOM_THEO_EMAIL(I_EMAIL);

        -- Sau ?ó, ti?n hành xóa ng??i dùng
...
IS
    I_MACV NUMBER; -- Khai báo bi?n I_MACV ? ?ây
BEGIN
    -- them macv tu dong
...
BEGIN
    FOR rec IN (
        ...
        FROM CONGVIEC CV
        JOIN THUCHIEN TH ON CV.MACV = TH.MACV_PC
        WHERE TH.TRANGTHAI = '?ã hoàn thành'
        )
    LOOP
...
BEGIN
    -- Xóa ng??i dùng kh?i b?ng THUCHIEN
    DELETE FROM THUCHIEN
    WHERE EMAIL_TV = p_email;

    -- Xóa ng??i dùng kh?i b?ng NGUOIDUNG_NHOM
    DELETE FROM NGUOIDUNG_NHOM
    WHERE EMAILND = p_email;

    -- Xóa ng??i dùng kh?i b?ng NGUOIDUNG
    DELETE FROM NGUOIDUNG
    WHERE EMAILND = p_email;

    COMMIT;
    DBMS_OUTPUT.PUT_LINE('?ã xóa ng??i dùng và các thông tin liên quan: ' || p_email);
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        RAISE;
